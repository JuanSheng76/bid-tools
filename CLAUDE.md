...
投标辅助工具 — 面向小团队（2-5人）的轻量化投标管理系统。目标行业：光伏检测、招标代理。
...
### UI 设计系统（2026-07 美学优化）
- **配色**：主色 `#4f46e5`（靛蓝紫），侧边栏 `#0f172a→#1e293b` 渐变，背景 `#f8fafc`
- **圆角**：卡片 16px，按钮 8px，徽章 5-6px
- **阴影层级**：sm(1px blur-4) / md(4px blur-16) / lg(8px blur-30)
- **统计卡片**：顶部 4px 彩色渐变条 + hover 上浮 3px + 加载计数动画
- **侧边栏**：激活项左侧 3px 紫色指示条 + hover 平滑过渡
- **看板列**：三列不同淡彩背景（淡蓝/淡橙/淡绿）+ 列标题数量徽章
- **表格**：斑马纹隔行 + 表头细线底框（非灰底）
- **按钮**：主按钮渐变背景 + hover 发光阴影，危险按钮 outline→实心
- **Header**：毛玻璃效果（`backdrop-filter: blur`）
- **空状态**：统一 `.empty-state` 组件（大图标 + 标题 + 引导文案）
- **筛选栏**：统一 `.filter-bar` 组件（白底 + 圆角 + 边框，`padding: 16px 20px`），用于所有列表页顶部筛选区
- **CSS 集中**：绝大部分样式在 `base.html` `<style>` 块，`style.css` 仅补充动画/拖拽/打印样式
